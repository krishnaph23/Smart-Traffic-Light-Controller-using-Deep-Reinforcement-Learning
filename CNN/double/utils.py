...
def import_train_configuration(config_file):
    ...
    config['n_low'] = content['simulation'].getint('n_low')
    config['n_high'] = content['simulation'].getint('n_high')
...
def set_sumo(gui, sumocfg_file_name, max_steps):
    ...
    sumo_cmd = [sumoBinary, "-c", os.path.join('intersection', sumocfg_file_name), "--no-step-log", "true", "-W", "true", "--waiting-time-memory", str(max_steps)]
